...
const TOAST_CLOSE_ICON = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>';
...
function showToast(message, type = 'error', duration, options = {}) {
  ...
  toast.innerHTML = `${TOAST_ICONS[type] || ''}<span class="toast-msg">${escapeHtml(displayMessage)}</span><button type="button" class="toast-close" aria-label="关闭提示" title="关闭提示">${TOAST_CLOSE_ICON}</button>`;
